...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (filterClear && filterChips.length > 0) {
        ...
        const getCategoryTag = (headerText) => {
            ...
            if (t.includes('愛撫')) return '愛撫';
            if (t.includes('指でほぐす')) return '指';
            if (t.includes('繋がる瞬間')) return '繋がる';
            if (t.includes('律動') || t.includes('快感')) return '律動';
            if (t.includes('体位')) return '体位';
            if (t.includes('絶頂') || t.includes('余韻')) return '絶頂';
            return null;
        };
