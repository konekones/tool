// これは販売パッケージ（通常版）と同等の見た目・挙動を確認するためのサンプル用JSです
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 通常版のみ：表示上の「（xx文字）」を除去（本文の読みやすさ重視）
    // ※HTMLデータ側に残っていても、ここで画面から消す
    const allDescItemsForDisplay = document.querySelectorAll('.desc-item');
    allDescItemsForDisplay.forEach(li => {
        li.innerText = (li.innerText || '').replace(/（\d+文字）/g, '').trim();
    });
    ...
    if (filterClear && filterChips.length > 0) {
        ...
        const getCategoryTag = (headerText) => {
            ...
            if (t.includes('軽いキス')) return '軽いキス';
            if (t.includes('普通のキス')) return '普通のキス';
            if (t.includes('少し長めのキス')) return '少し長めのキス';
            if (t.includes('ディープキス')) return 'ディープキス';
            if (t.includes('初めてのキス')) return '初めてのキス';
            if (t.includes('不意打ちのキス')) return '不意打ちのキス';
            if (t.includes('別れ際のキス')) return '別れ際のキス';
            if (t.includes('壁ドン')) return '壁ドン';
            if (t.includes('甘えるようなキス')) return '甘えるようなキス';
            if (t.includes('久々の再会')) return '久々の再会';
            return null;
        };

        const getViewTag = (itemText) => {
            const t = (itemText || '');
            if (t.includes('【される】')) return 'される';
            if (t.includes('【する】')) return 'する';
            if (t.includes('【両対応】')) return '両対応';
            return null;
        };
